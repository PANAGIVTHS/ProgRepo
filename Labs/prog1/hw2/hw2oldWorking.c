...
int testforNULL (char* ptrWordArray[][2],int index) {
    if (index<MAX_WORDS) { 
        //Avoid scanning the whole array in case of EOF
        if (ptrWordArray[index][0] == NULL) return(1);
    }
    return(0);
}

void scanText (char wordArray[][MAX_WORD_LEN], char* ptrWordArray[][2]) {
    char formatStr[20], temp[MAX_WORD_LEN] = {'\0'};
...
void printText (char *ptrWordArray[][2], int linelen) {
    ...
    for (int i = 0; i < MAX_WORDS; i++) {
        ...
        // If statements cant go inside for's logic statement because of the last loop 
        // being segm fault
        if (testforNULL(ptrWordArray, i)) break;
        
        if(strcmp(ptrWordArray[i][0], ".") == 0 || strcmp(ptrWordArray[i][0], "?") == 0 || strcmp(ptrWordArray[i][0], "!") == 0) {
            nextToUpper = 1;
        }
        if(nextToUpper) {
            ptrWordArray[i+1][0][0] = toupper(ptrWordArray[i+1][0][0]);
        }
        ...
        else printf("%s ", ptrWordArray[i][0]);
    }
    printf("\n");
}
...
void findFrequency (char *ptrWordArray[][2], int frequencyArray[]) {
    ...
    for (int i = 0, k = 0; i < MAX_WORDS && k<MAX_WORDS; i++) {
        ...
        for (int j = k; j<MAX_WORDS; j++) {
            if (testforNULL(ptrWordArray, k)) break;
            if (testforNULL(ptrWordArray, j)) break;

            if (strcmp(ptrWordArray[k][0],ptrWordArray[j][0]) == 0) {
                //printf("%d|%d-->Test%s\n", k,j,ptrWordArray[k][0]);
                frequencyArray[(ptrWordArray[j][0] - ptrWordArray[0][0])/20]++;
            }
        }
        ...
        for (; k<MAX_WORDS; k++) {
            ...
            if ((ptrWordArray[k][0] - ptrWordArray[0][0])/20 > oldDistance) {
                //printf("Yes %ld.", (ptrWordArray[k][0] - ptrWordArray[0][0])/20);
                newDistance = (ptrWordArray[k][0] - ptrWordArray[0][0])/20;
...
int findFrequentWords (char wordArray[][MAX_WORD_LEN], char *ptrWordArray[][2], char mostFreqWordsArray[][MAX_WORD_LEN]) {
    ...
    for (int i = 0; (i < MAX_WORDS) && (currentMax < MAX_WORDS - indexesSum); i++) {
        indexesSum += frequencyArray[i];
        if (currentMax == frequencyArray[i]) {
...
void printWords (char wordArray[][MAX_WORD_LEN]) {
    ...
    numOfWords = doesStrExist(wordArray, "\0", MAX_WORDS);
    for (int i = 0; i < numOfWords; i++) {
        printf("%s\n", wordArray[i]);
    }
}

void findSynonyms (char *ptrWordArray[][2], char synonymsArray[][MAX_WORD_LEN]) {
    // used to stop searching when out of synonyms
    int usedSynonyms[MAX_WORDS] = {0};
    int outOfSynonyms;
    
    for (int i = 0; i < MAX_WORDS && (testforNULL(ptrWordArray, i) == 0); i++) {
        outOfSynonyms = 0;
        for (int j = 0; j < MAX_WORDS && (synonymsArray[j][0] != '\0'); j++) {
            ...
            if (testforNULL(ptrWordArray, i+1) || outOfSynonyms) break;
        }
    }
}
...
int main(int argc, char *argv[]) {
    ...
    char synonymsArray[MAX_WORDS][MAX_WORD_LEN] = {{'a','\0','A','\0'},{'g','\0','G','\0'}};

    for (int i = 1; i < argc; i++) {
        // i=1 cause argv[1] is ./hw2
        if (strcmp(argv[i], "-r") == 0) runR = 1;
        if (strcmp(argv[i], "-m") == 0) runM = 1;
        if (strcmp(argv[i], "-p") == 0) runP = 1;
        if (runM+runP+runR==3) break;
    }
    /* Same as 3 OR */
    if (runM+runP+runR == 0) {
