...
/* 
    This programme takes TEXT as input and stores it some what efficiently (in terms of memory).
    Also finds basic info about the text (most Frequent Words, how many times they appear)
    there is an option that finds how much memory is saved (%)
*/
    ...
    for (int j = 0, i = 0; (j<MAX_WORDS) && (i < MAX_WORDS); j++) {
        ...
        if (scanf(formatStr, temp) != EOF) {
            ...
            if (doesExist != -1) {
                ...
                // if it doesnt exist write in new index
...
void printText (char *ptrWordArray[][2], int linelen) {
    ...
    for (int i = 0; i < MAX_WORDS && (ptrWordArray[i][0] != NULL); i++) {
        ...
        // if both the word and its synonym dont fit 
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (strcmp(argv[i], runParameters[0]) == 0) {
            runR = i;
            isParameter = 1;
        }
        if (strcmp(argv[i], runParameters[1]) == 0) {
            runM = i;
            isParameter = 1;
        }
        if (strcmp(argv[i], runParameters[2]) == 0) {
            runP = i;
            isParameter = 1;
        }
        if (strcmp(argv[i], runParameters[3]) == 0) {
            runS = i;
            isParameter = 1;
        }
        // check if there is a a wrong statement
        if (isParameter == 0 && (i != runR+1)) {
            runM = 0;
            runR = 0;
            runS = 0;
            runP = 0;
