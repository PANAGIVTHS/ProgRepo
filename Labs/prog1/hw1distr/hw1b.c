...
// tupou int afou jerw oti 8a parei mono powers of 2 mylog2(Valid Canditate number) basically 
int mylog2 (int value);

int main (int argc, char *argv[]) {

	int value, isPowOfTwo, tempMostVotes, winner, votedByMen, votedByWomen, votedByOther;
    ...
    long int ageRangeArray[AGE_GROUPS] = {0};
	int excessVotes=0, mostVotesOverall, mostVotesMen, mostVotesWomen, mostVotesOther, ageCase, printUntil;
	...
	for (int voterCounter = 0; voterCode == 0 || voterCounter < MAX_VOTERS; voterCounter++) {
		do {
			...
			scanf(" %hx", &voterCode);
			if (voterCode == 0) { 
                printUntil = voterCounter;
                break;
            }
            else {
                printUntil = MAX_VOTERS;
            }
			...
			isPowOfTwo = isPowerOfTwo(value);

			excessVotes = 0;
			for (int i = 0; (i < INDEX_BEFORE_GENDER); i++) {
                // checks if the LSB is a 1
				if ((((voterDataArray[voterCounter][2] >> i) & maskNumOfCand) == maskNumOfCand) && (isPowOfTwo == 0)) { 
            ...
            else if ((voterDataArray[voterCounter][1] != 0) && ((isPowOfTwo == 1) || voterDataArray[voterCounter][2] == 0)){
			...
			if ((voterDataArray[voterCounter][2] != 0) && (isPowOfTwo == 0)) {
				printf("Invalid votes %d.\n", excessVotes + 1);
			}
            if (isPowOfTwo && voterDataArray[voterCounter][1] != 0 && voterDataArray[voterCounter][0] >= 18) {
                votesArray[mylog2(voterDataArray[voterCounter][2])][voterDataArray[voterCounter][1]]++; // Add vote based on gender. voterDataArray[voterCounter][1] values can be 1,2,3
                votesArray[mylog2(voterDataArray[voterCounter][2])][0]++; // Add to overall votes
            }
            if (voterDataArray[voterCounter][2] == 0) {
                voterDataArray[voterCounter][2] = 0;
            }
		} while ((((isPowOfTwo == 0) && (voterDataArray[voterCounter][2] != 0)) || (voterDataArray[voterCounter][1] == 0) || (voterDataArray[voterCounter][0] < 18)) && voterCode != 0);    
        if (voterCode == 0) {
    ...
    for (int voterCounter = 0; voterCounter < MAX_VOTERS; voterCounter++) {
        if (voterDataArray[voterCounter][2] != 0) {
            voterDataArray[voterCounter][2] = mylog2(voterDataArray[voterCounter][2]) + 1; // +1 to differenciate canditate #0 and non-vote 0
        }
    }
    do {
        // Prints menu
...
int isPowerOfTwo (int value) {
	...
	if (value != 1) {
		while (value != 1) {
	    	...
	    	result = value % 2;  // <--- Personal comment. i LOST my mind over this.
	        value /= 2;
	    	if ((result != 0)|| value == 0) {
	    		return(0);
	            break;
